...
export class Locomotion {
  ...
  pose={tilt:0,bow:0,wave:0,crouch:0,breath:0};
  yaw=0;
  phase=0;
  ...
  step(h:number) {
    ...
    b.canSleep=speed<.001&&!this.jumpQueued&&Object.values(this.pose).every(v=>Math.abs(v)<.0001);
    ...
    for(let i=0;i<b.mass.length;i++) {
      const j=i*3;
      let rx=b.rest[j]-this.restCenter.x;
      ...
      const head=Math.max(0,Math.min(1,(b.rest[j+1]-.036)/.024));
      const originalX=rx;
      rx+=this.pose.tilt*head*.012;
      ry+=this.pose.breath*head*.0007-this.pose.crouch*(1-foot)*.003;
      rz+=this.pose.bow*head*.005;
      if(originalX>0){ry+=arm*this.pose.wave*.028;rx-=arm*this.pose.wave*.003;}
