...
export class RoomView {
  ...
  buildRoom(){
    ...
    for(let i=0;i<34;i++){this.box(.445,.025,9.8,i%3===0?'#d8c6a7':i%3===1?'#e2d2b6':'#dfceb0',-7.43+i*.45,.035,0,.002);}
    ...
    this.box(5.2,.025,4.5,'#b0b9a2',-4,.065,.2,.12);
    for(let i=0;i<6;i++)this.box(4.9,.006,.015,'#d1d5c5',-4,.082,-1.8+i*.78,.001);
    this.box(6.15,.032,9.2,'#b6c9c4',4.12,.06,0,.13);
  ...
  async loadCharacter(){
    this.avatarSystem=new OriginalCharacter();this.avatar=this.avatarSystem.root;this.model=this.avatar;this.bones=this.avatarSystem.bones;this.scene.add(this.avatar);this.ready=true;
    return {source:'Original procedural geometry and motion',bones:Object.keys(this.bones),animations:['idle','walk','run','reach','sit','stand','drive']};
  }
  ...
  close(){if(!this.avatar)return;const p=this.avatar.position;this.camera.position.set(p.x+2.8,2.7,p.z+4.3);this.controls.target.set(p.x,1.0,p.z);this.follow=true;this.frameLens();}
