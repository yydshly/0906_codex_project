...
const BODY=[[.861,.001,.001],[.865,.215,.137],[.9,.222,.143],[1.02,.204,.133],[1.15,.207,.137],[1.30,.242,.153],[1.38,.249,.148],[1.43,.224,.130],[1.49,.103,.080],[1.50,.085,.071]];
...
export class OriginalCharacter {
  constructor(){
    ...
    this.hair=this.material('#49332c',.68,{side:THREE.DoubleSide});this.hairLight=new THREE.MeshBasicMaterial({color:'#513a30',side:THREE.DoubleSide});
  ...
  limb(upperName,lowerName,lenA,lenB,radii,material){
    ...
    const g=surface(40,24,(u,v)=>{
      const t=1-u,ri=t*(radii.length-1),i=Math.floor(ri),r=THREE.MathUtils.lerp(radii[i],radii[Math.min(i+1,radii.length-1)],ri-i),a=v*Math.PI*2;
    ...
    this.skinMesh(g,material,p=>{const t=smooth(lenA-.07,lenA+.07,origin.y-p.y);return [[upper,1-t],[lower,t]];},upperName+'_skin');
  }

  buildBody(){
    const b=this.bones;
    this.skinMesh(profileGeometry(BODY),this.shirt,p=>this.torsoWeights(p),'tailored_overshirt');
    ...
    for(const side of ['r','l']){
      this.ellipsoid(.098,.084,.085,this.shirt,b['upperarm'+side],0,-.003,0);
      this.limb('upperarm'+side,'lowerarm'+side,.32,.31,[.092,.098,.085,.076,.077,.075,.063,.058],this.shirt);
      this.limb('upperleg'+side,'lowerleg'+side,.39,.38,[.112,.116,.106,.088,.078,.087,.072,.061],this.trousers);
      ...
      this.box(.155,.115,.282,this.cream,foot,0,-.035,.062,.037);
      this.box(.165,.032,.295,this.sole,foot,0,-.090,.065,.012);
      this.box(.106,.042,.143,this.shirt,foot,0,.013,.062,.012);
      for(let i=0;i<3;i++)this.curve([[-.043,.037,.06+i*.033],[0,.043,.067+i*.033],[.043,.037,.06+i*.033]],.003,this.cream,foot);
      this.box(.075,.057,.014,this.shirt,foot,0,-.005,-.069,.007);
    }
  }

  buildHead(){
    const head=this.bones.head,base=new THREE.Color('#dfb59e'),blush=new THREE.Color('#c78472');
    const faceProfile=[[-.232,.016,.037,0],[-.205,.083,.075,.009],[-.16,.143,.112,.003],[-.085,.182,.141,0],[-.015,.194,.154,-.005],[.07,.188,.15,-.004],[.16,.179,.153,-.009],[.23,.128,.122,-.012],[.273,.003,.004,-.012]];
    ...
    for(const sign of [-1,1]){
      this.ellipsoid(.03,.047,.022,this.skin,head,sign*.189,-.025,-.014);
      ...
      this.ellipsoid(.042,.023,.017,this.cream,eye);
    ...
    for(let i=0;i<11;i++){
      ...
      this.curve(points,.0012,this.hairLight,head);
    }
    const locks=[
    ...
    for(const lock of locks)this.mesh(ribbonGeometry(lock.p,lock.w,lock.d),this.hair,head);
    for(const [i,lock] of locks.entries()){
      const pts=lock.p.map(([x,y,z],k)=>[x+(i%2?.008:-.008),y+.004,z+(k===0?.003:.014)]);
      this.curve(pts,.0017,this.hairLight,head);
    }
  }
